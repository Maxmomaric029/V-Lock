...
namespace AutoRescan {
    ...
    std::atomic<int64_t> lastStateChange{ 0 };
    std::atomic<int64_t> lastMemoryCheck{ 0 };
}
struct GameState {
    uint64_t gameId = 0;
    uint64_t placeId = 0;
...
bool QuickMemoryCheck() {
    ...
    auto lastCheck = std::chrono::steady_clock::time_point(std::chrono::steady_clock::duration(AutoRescan::lastMemoryCheck.load()));
    ...
    AutoRescan::lastMemoryCheck = now.time_since_epoch().count();
...
GameState CaptureCurrentState() {
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(g_smartCache.cacheMutex);
    ...
    try {
        ...
        std::lock_guard<std::recursive_mutex> instanceLock(g_instanceMutex);
...
bool PerformSmartRescan(GameState::StateType expectedState) {
    try {
        ...
        std::lock_guard<std::recursive_mutex> instanceLock(g_instanceMutex);		std::uint64_t fake_datamodel = memory->read<std::uint64_t>(memory->get_module_address() + Offsets::FakeDataModel::Pointer);
		if (!fake_datamodel || fake_datamodel > 0x7FFFFFFFFFFFFFFF)
		{
			return false;
		}
    ...
    while (AutoRescan::active) {
        try {
            ...
            if (isTransition) {
                ...
                AutoRescan::lastStateChange = std::chrono::steady_clock::now().time_since_epoch().count();
            }
            else {
                auto now = std::chrono::steady_clock::now();
                auto lastChange = std::chrono::steady_clock::time_point(std::chrono::steady_clock::duration(AutoRescan::lastStateChange.load()));
                auto timeSinceChange = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastChange);
            ...
            if (hasAnyChange || stateTypeChanged || forceRescan || requiresImmediate) {
                ...
                if (timeSinceLastRescan.count() >= requiredCooldown || forceRescan || requiresImmediate) {
                    std::lock_guard<std::recursive_mutex> lock(AutoRescan::updateMutex);
                    ...
                    if (rescanSuccess) {
                        std::lock_guard<std::recursive_mutex> cacheLock(g_smartCache.cacheMutex);
                        g_smartCache.isValid = false;
...
void AutoRescanHandler() {
    ...
    AutoRescan::lastStateChange = std::chrono::steady_clock::now().time_since_epoch().count();
    AutoRescan::lastMemoryCheck = std::chrono::steady_clock::now().time_since_epoch().count();
